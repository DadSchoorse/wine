...
#define WINE_VK_ADD_HANDLE_MAPPING(instance, object, native_handle)\
    wine_vk_add_handle_mapping((instance), (uint64_t) (object), (uint64_t) (native_handle), &(object)->mapping)
        ...
        VkPhysicalDevice phys_dev)
{
    ...
    WINE_VK_ADD_HANDLE_MAPPING(instance, object, phys_dev);
        ...
        uint32_t family_index, uint32_t queue_count, VkDeviceQueueCreateFlags flags)
{
    ...
    for (i = 0; i < queue_count; i++)
    {
        ...
        WINE_VK_ADD_HANDLE_MAPPING(device->phys_dev->instance, queue, queue->queue);
    }
        ...
        const VkCommandBufferAllocateInfo *allocate_info, VkCommandBuffer *buffers)
{
    ...
    for (i = 0; i < allocate_info->commandBufferCount; i++)
    {
        ...
        WINE_VK_ADD_HANDLE_MAPPING(device->phys_dev->instance, buffers[i], buffers[i]->command_buffer);
        if (res != VK_SUCCESS)
        {
        ...
        const VkAllocationCallbacks *allocator, VkDevice *device)
{
    ...
    WINE_VK_ADD_HANDLE_MAPPING(phys_dev->instance, object, object->device);
    if (res != VK_SUCCESS)
    {
